...
// Konfigurasi koneksi MySQL Server.
// Lokal (XAMPP): biarkan .env kosong / tidak ada, otomatis fallback ke default XAMPP.
// Production (Render dsb.): isi DB_HOST, DB_USER, DB_PASSWORD lewat environment variables.
const dbConfig = {
...
const DB_NAME = process.env.DB_NAME || 'posturography';
let dbPool = null;

async function initializeDatabase() {
  try {
    // 1. Hubungkan ke MySQL Server tanpa memilih database terlebih dahulu, lalu
    //    coba buat database jika belum ada (jalan mulus di XAMPP lokal).
    //    Beberapa provider MySQL managed/gratis tidak mengizinkan CREATE DATABASE
    //    dari akun biasa (database sudah disediakan lewat dashboard mereka),
    //    jadi kegagalan di langkah ini tidak menghentikan server.
    try {
      const tempConnection = await mysql.createConnection(dbConfig);
      await tempConnection.query(`CREATE DATABASE IF NOT EXISTS \`${DB_NAME}\``);
      console.log(`Database "${DB_NAME}" terverifikasi / dibuat.`);
      await tempConnection.end();
    } catch (createErr) {
      console.warn(`⚠️ Tidak bisa memastikan database "${DB_NAME}" otomatis (${createErr.message}). Pastikan database sudah ada di provider Anda.`);
    }

    // 2. Buat Connection Pool untuk database yang dipakai
...
// ==========================================
// Catatan: Endpoint koneksi serial hardware (SerialPort Node.js) yang lama
// sudah dihapus dari sini karena tidak pernah dipanggil oleh frontend —
// koneksi ke Arduino dilakukan langsung dari browser lewat Web Serial API
// (lihat frontend/src/utils/webSerial.js). Ini juga membuat backend lebih
// ringan untuk di-deploy karena tidak lagi butuh native module `serialport`.
// ==========================================
